<!DOCTYPE html>
<html lang="pt-br">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Array</title>
</head>
<body>
    <h1>Arrays (vetores e matrizes)</h1>
    <hr>

<h2>Arrays numéricos/indexados </h2>

<?php

// Declarando arrays 

$bandas= ["Rush","Slayer","Iron Maiden"];
$cursos= array ("HTML5","PHP","Design","JS");

// Declarando como constante 


define("UNIDADES_SESC",["Belenzinho","Bertioga"]);
const UNIDADES_SENAC = ["Penha","Tatuapé","São Miguel","Itaquera"];


?>

<h3>Saída de dados de arrays</h3>

<ul>

<li>Banda que mais gosto: <?=$bandas[0] ?></li>
<li>Assunto estudado : <?=$cursos [1] ?></li>

</ul>

<!-- Mini-exercício 

- mostre em que unidade do Senac estamos 
- mostre também o Sesc que fica no litoral -->

<ul>

<li>Unidade que estamos é no Senac <?= UNIDADES_SENAC [0]?>


</li>
<li>Unidade que Sesc tem no litoral é <?=UNIDADES_SESC [1]?></li>

</ul>

<hr>

<h2>Array associativo</h2>

<?php

// chave associativa 

$livro= [ 

    "titulo"=> "Senhor dos Anéis",
    "autor"=> "J.R.R. Tolkian",
    "ano"=> 1954,
    "personagens" => ["Frodo","Legodas"]
];

$filme= array (

    "titulo" => "Velozes e Furiosas",
    "genero" => "Ação"


);



?>

<h3>Saída de dados (Array associativo)</h3>

<p> Livro: <?=$livro["titulo"]?> </p>
<p> O cara que usa fechinhas : <?=$livro["personagens"][1]?></p>

<hr>

<h3>Comandos para analisar a estrutura de arrays :print_r() e var_dump() </h3>

<!-- Saídas sem formatação -->

<?=print_r($bandas)?>

<br><br>

<?=var_dump($bandas)?>

<!-- Saídas com formatação usando a tag <pre>-->

<pre> <?=print_r($bandas)?> </pre>
<pre> <?=var_dump($bandas)?></pre>
<pre> <?=var_dump($livro)?></pre>



</body>
</html>